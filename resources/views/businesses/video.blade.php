...
@section('page_class')
    edit-business-video edit-business businesses @parent
@stop
        ...
        <div class="container white-content-block">
            <div class="row">
                <div class="col-md-12 content vessel-content mt-4 mb-4 text-center">
                    @php($link = $business->attachments()->where('type', 'video')->first())
                    @if ($link)
                        ...
                        <div>
                            <a href="{{ route('account.businesses.profile.video.delete', ['boat_id' => $business->id]) }}" class="btn btn--orange mt-3" onclick="return confirm('Are you sure you want to delete video?');">@lang('button.delete')</a>
                        </div>
                    @else
    ...
    <script>
        $(function () {
            $('#fileupload').fileupload({
                url: "{{ route('account.businesses.profile.video.store', ['business_id' => $business->id]) }}",
                dataType: 'json',
                autoUpload: true,
                ...
                fail: function (e, data) {
                    ...
                    if (file.error) {
                        ...
                        window.location.href = "{{ route('account.businesses.profile.video', ['business_id' => $business->id]) }}";
                    }
                },
