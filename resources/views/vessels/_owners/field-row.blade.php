<div class="container-fluid data-row mb-3" {!! isset($index) ? 'data-index="' . $index . '"' : '' !!}>
    <div class="row">
        ...
        <div class="col-4 {{ isset($index) ? $errors->first('owners.' . $index . '.email', 'has-error') : null }}">
    ...
    <div class="row mt-2">
        <div class="col-3 {{ isset($index) ? $errors->first('owners.' . $index . '.phone', 'has-error') : null }}">
            <input type="text" data-name="owners[*][phone]" {!! isset($index) ? 'name="owners[' . $index . '][phone]"' : '' !!} autocomplete="off" class="form-control data-phone" value="{{ $phone ?? '' }}" placeholder="Cell #">
            @if(isset($index))
                {!! $errors->first('owners.' . $index . '.phone', '<span class="help-block">:message</span>') !!}
            @endif
        </div>
        <div class="col-3 {{ isset($index) ? $errors->first('owners.' . $index . '.phone_home', 'has-error') : null }}">
            <input type="text" data-name="owners[*][phone_home]" {!! isset($index) ? 'name="owners[' . $index . '][phone_home]"' : '' !!} autocomplete="off" class="form-control data-phone-home" value="{{ $phone_home ?? '' }}" placeholder="Home #">
            @if(isset($index))
                {!! $errors->first('owners.' . $index . '.phone_home', '<span class="help-block">:message</span>') !!}
            @endif
        </div>
    </div>
</div>