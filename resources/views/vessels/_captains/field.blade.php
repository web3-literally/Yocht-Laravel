@php($captains = old('captains') ?? $model->staff ?? [])
<div class="captains-field data-field">
    <div class="template" style="display:none">
        @component('vessels._captains.field-row')
        @endcomponent
    </div>
    <div class="data-rows">
        @foreach($captains as $index => $owner)
            @component('vessels._captains.field-row')
                @slot('index')
                    {{ $index }}
                @endslot
...
@section('footer_scripts')
    ...
    <script>
        $(function() {
            $('.captains-field.data-field').each(function(i, el) {
                var field = $(el);
