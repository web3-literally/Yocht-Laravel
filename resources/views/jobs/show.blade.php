...
@section('page_class')
    job-public job-details event-details jobs @parent
@stop
                ...
                <div class="container content">
                    ...
                    @if(!$charged)
                        ...
                        <div class="text-center">
                            <button type="button" class="btn btn-charge btn--orange" data-action="{{ route('jobs.show.charge', ['slug' => $job->slug]) }}">View Details ({{ Amsgames\LaravelShop\LaravelShop::format(config('billing.vessel.extra_view_private_job_details_cost')) }})</button>
                        </div>
                    @endif
                    <hr>
                    <div class="info-block">
                        <div class="posted-by">
                            ...
                            {{--on {{ $job->updated_at->toFormattedDateString() }}--}}
                            {{--in <span>{{ $job->category->label }}</span>--}}
                        </div>
                        <div class="right-share-info">
                            <div class="banner-share">
                                <span> <i class="fas fa-link"></i>Like This, Share it:</span>
                                @widget('ShareIcons')
                            </div>
                            <div class="apply-btn">
                                {{--@if($job->user_id != Sentinel::getUser()->getUserId())
                                    @php($reviewUrl = route('jobs.review', ['id' => $job->id]))
                                    <a href="{{ $reviewUrl }}" class="write-review btn btn--orange mr-1">@lang('reviews.post_a_review')</a>
                                @endif--}}
                                @if($charged)
                                    <a href="{{ route('jobs.apply-form', $job->slug) }}" class="btn btn--orange">@lang('jobs.apply_for_this_job')</a>
                                @endif
                            </div>
                        </div>
                    </div>
