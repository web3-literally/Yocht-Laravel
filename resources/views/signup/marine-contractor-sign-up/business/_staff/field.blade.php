<div class="staff-field data-field">
    <div class="template" style="display:none">
        @component('signup.marine-contractor-sign-up.business._staff.field-row')
        @endcomponent
    </div>
    <div class="data-rows">
        @foreach($staff as $index => $item)
            @component('signup.marine-contractor-sign-up.business._staff.field-row', ['business_index' => $business_index ?? null])
                @slot('index')
                    {{ $index }}
                @endslot
                @slot('name')
